...
def upgrade() -> None:
    ...
    subscription_tier_enum = sa.Enum(
        "free", "individual", "organization", name="subscriptiontier"
    )
    ...
    op.add_column(
        "users", sa.Column("subscription_started_at", sa.DateTime(), nullable=True)
    )
    op.add_column(
    ...
    op.add_column(
        "users",
        sa.Column("is_org_admin", sa.Boolean(), nullable=True, server_default="false"),
    )
