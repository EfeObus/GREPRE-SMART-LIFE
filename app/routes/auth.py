...
async def register(
    ...
    create_samples: bool = Query(
        True, description="Create sample bills and documents for new user"
    ),
...
async def login(
    ...
    if not user or not verify_password(user_data.password, user.hashed_password):
        ...
        is_now_locked, lockout_seconds = await lockout_manager.record_failed_attempt(
            email, client_ip
        )
...
async def refresh_access_token(
    ...
    if not user:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail={"code": ErrorCode.AUTH_INVALID_TOKEN, "message": "User not found"},
        )
