...
from app.core.exceptions import (
    NotFoundError, AccessDeniedError, ValidationError, TransactionError,
    ErrorCode, create_success_response
)
...
async def get_user_bill(
    ...
    if bill.user_id != user_id:
        raise HTTPException(
            ...
            detail={"code": ErrorCode.BILL_ACCESS_DENIED, "message": "You do not have access to this bill"}
        )
...
async def create_bill(
    ...
    try:
        ...
        raise HTTPException(
            ...
            detail={"code": ErrorCode.TRANSACTION_FAILED, "message": f"Failed to create bill: {str(e)}"}
        )
...
async def update_bill(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"code": ErrorCode.TRANSACTION_FAILED, "message": f"Failed to update bill: {str(e)}"}
        )
...
async def delete_bill(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"code": ErrorCode.TRANSACTION_FAILED, "message": f"Failed to delete bill: {str(e)}"}
        )
...
async def restore_bill(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"code": ErrorCode.TRANSACTION_FAILED, "message": f"Failed to restore bill: {str(e)}"}
        )
...
async def pay_bill(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"code": ErrorCode.TRANSACTION_FAILED, "message": f"Failed to record payment: {str(e)}"}
        )
...
async def get_bill_payments(
    ...
    result = await db.execute(
        select(Payment).where(Payment.bill_id == bill_id).order_by(Payment.paid_date.desc())
    )
