...
from app.models import Bill, BillDocumentLink, Document, Payment, User
...
async def export_user_data(
    ...
    docs_query = (
        select(Document)
        .options(selectinload(Document.linked_bills))
    ...
    for doc in documents:
        ...
        if include_relationships:
            doc_data["linked_bill_ids"] = [link.bill_id for link in doc.linked_bills]
...
def export_user_profile(user: User) -> Dict[str, Any]:
    ...
    return {
        ...
        "default_bill_category": serialize_enum(
...
async def export_to_csv_format(
    user: User, db: AsyncSession, data_type: str = "bills"
) -> List[Dict[str, Any]]:
    """
